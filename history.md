...
- **fix(gui): 🎨 修复重新获取歌曲数据后封面图片组件主题颜色未更新的问题**
  ...
  - **修复方案**：
    1. **为 CoverImageWidget 添加主题变化监听**：
       - 在 `__init__()` 中连接 `qconfig.themeChanged` 信号
       - 新增 `_on_theme_changed()` 方法，主题切换时调用 `_setup_style()` 更新样式
    2. **确保新创建的组件能正确响应主题切换**
  - **效果**：
    - ✅ 重新获取歌曲数据后，封面图片组件能正确响应主题切换
    - ✅ 深浅色模式下背景色和边框色正确更新
    - ✅ 不影响现有功能
  - **涉及文件**：
    - `auto_tag/gui/components/song_result_card.py` - CoverImageWidget 添加主题监听
...
- **feat(core): 🔍 优化文件名识别策略 - 不像歌曲名时优先 Shazam 音频识别**
  ...
  - **修复方案**：
    1. **新增 `_is_filename_like_song_name()` 函数**：
       - 规则1: 检测数字+下划线+数字模式（如 32671414_da3-1-30216）
       - 规则2: 数字占比超过 70% 的文件名
       - 规则3: 包含无意义关键词（download, temp, rec, recording 等）
       - 规则4: 文件名过长（>50 字符）且无分隔符
       - 规则5: 仅包含特殊字符
       - 支持中文、日文、韩文等多语言字符识别
    2. **优化 `recognize_and_rename_file()` 搜索策略**：
       - 文件名不像歌曲名时，优先使用 Shazam 音频识别获取真实音乐信息
       - Shazam 失败后，只有文件名像歌曲名时才使用文件名作为关键词回退搜索
       - 避免使用无意义文件名进行搜索
  - **效果**：
    - ✅ 随机 ID 文件名优先通过 Shazam 识别获取正确音乐信息
    - ✅ 正常歌曲文件名（如"周杰伦 - 晴天.mp3"）保持原有搜索逻辑
    - ✅ 12 个单元测试全部通过
    - ✅ 8 个核心识别测试全部通过
  - **涉及文件**：
    - `auto_tag/audio_recognize.py` - 新增文件名判断函数，优化搜索策略
    - `tests/test_filename_check.py` - 新增单元测试
...
- feat(ui): 优化窗口尺寸并添加手动调整大小功能
  ...
  - 添加手动调整窗口大小功能
    - 设置最小尺寸：900×500（防止内容截断）
    - 设置最大尺寸：1920×1080（适配大屏显示器）
    - 用户可自由拖拽边框调整窗口大小
  - 宽高比约 2.07:1，保持扁平化视觉效果

## v0.4.25 (2026-04-22)
- chore(tool): 将虚拟环境从 venv 迁移到 uv
  - 安装 uv 0.7.13（基于 Rust 的高性能 Python 包管理器）
  - 使用 `uv venv` 创建 .venv 虚拟环境（替代原 venv）
  - 使用 `uv sync --all-extras` 同步所有依赖（生产+开发）
  - 更新 pyproject.toml：requires-python 从 ">=3.6" 改为 ">=3.10"（适配 shazamio 最新版本要求）
  - 验证通过：核心依赖（shazamio 0.8.1, PySide6 6.11, pytest 9.0.3）均正确安装
  - 更新 program.md 文档：添加 uv 常用命令说明，替换原有的 venv/pip 工作流
  - **性能提升**：uv 比 pip/venv 快 10-100 倍，依赖解析更智能可靠
